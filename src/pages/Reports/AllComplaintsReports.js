...
const AllComplaintsReports = (props) => {
     ...
     //      if (status == "All") {
     //           setFilteredComplaints([...complaintReport]);
     //      } else {
     //           let arr = complaintReport.filter(
     ...
     const loadComplaintReport = async (e, v) => {
          preloader(true)
          let url = process.env.REACT_APP_BASEURL + "reports/complaint_status_report";
          ...
          if (response && response.status === 1) {
               ...
               setComplaintReport(response.data);
                    ...
                    options: () => {
                         const unique = [
                              ...new Map(
                                   [
                                        ...new Set(
                                             complaintReport.map(({ status: value }) => ({
                                                  value: value,
                                                  label: value,
                                             }))
                                        ),
     ...
     const handleDownload = async () => {
          const fileName = "complaints_report";
          const exportType = "xls";
          var data = JSON.parse(JSON.stringify(complaintReport));
               ...
               <div className="page-content">
                    <Container fluid={true}>
                         <Breadcrumbs
                              ...
                              breadcrumbItem={props.t("All Complaints Report")}
                         />
                         <Row>
                              <Col lg={6}>
                                   <Card>
...
export default withRouter(
     connect(null, {})(withNamespaces()(AllComplaintsReports))
);
